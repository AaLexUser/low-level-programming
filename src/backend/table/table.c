...
int64_t tab_select_op(int64_t mtabidx,
   ...
   /* Load table */
    ...
    tab_for_each_row(sel_tab, sel_tabidx, sel_chblix, el_row, sel_schema){
        ...
        if(comp_compare(type, el,comp_val, condition)){
...
int tab_drop(int64_t mtabidx, int64_t tablix){
    if (mtab_delete(mtabidx, tablix) == TABLE_FAIL) {
...
int tab_update_row_op(int64_t tablix,
    ...
    /* Update */
    tab_for_each_row(upd_tab, tablix, upd_chblix, el_row, upd_schema){
...
int tab_delete_op(int64_t tablix,
    ...
    void* el_row = malloc(del_schema->slot_size);
    void* el = malloc(comp_field.size);
    void* comp_val = malloc(comp_field.size);
    memcpy(comp_val, value, comp_field.size);

    /* Delete */
    tab_for_each_row(del_tab, tablix, del_chblix, el_row, del_schema){
        memcpy(el, (char*)el_row + comp_field.offset, comp_field.size);
        if(comp_compare(comp_field.type, el, comp_val, condition)){
            if(tab_delete(tablix, &del_chblix) == TABLE_FAIL){
